  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (creadorReporte === emailUsuario) {
        misReportes.push({
          id: row[0],
          fecha: row[1] ? Utilities.formatDate(new Date(row[1]), Session.getScriptTimeZone(), "dd/MM/yyyy") : "",
          empresa: row[2],
          tienda: row[3],
          posicion: row[4],
          tipoReporte: row[5],
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === idReporte && (data[i][13] || "").toString().toLowerCase() === miCorreo) {
        ...
        hoja.getRange(i + 1, 3).setValue(datos.empresa);       // Col C
        hoja.getRange(i + 1, 4).setValue(datos.tienda);        // Col D
        hoja.getRange(i + 1, 5).setValue(datos.posicion);      // Col E
        hoja.getRange(i + 1, 6).setValue(datos.tipoReporte);   // Col F
        hoja.getRange(i + 1, 7).setValue(datos.empleado);      // Col G
        hoja.getRange(i + 1, 8).setValue(datos.iniciales);     // Col H
        hoja.getRange(i + 1, 9).setValue(datos.observaciones); // Col I
        hoja.getRange(i + 1, 10).setValue("Pendiente");        // Col J
        hoja.getRange(i + 1, 12).setValue("");                 // Limpiar Obs Supervisor
        ...
        if (files && files.length > 0) {
          ...
          if (urlCarpeta && urlCarpeta.includes("drive.google.com/drive/folders/")) {
             ...
             } catch(e) { Logger.log("Aviso en actualización de archivos: " + e.message); }
          }
        }
