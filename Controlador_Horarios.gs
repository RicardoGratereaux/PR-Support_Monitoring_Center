    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][2] === nombre) {
        ...
        resultados.push({
          ...
          llegadaReal: extraerHora(data[i][10]) || ""
        });
      }
    }
...
function predecirHorarioColaborador(nombre, fechaStr) {
  try {
    ...
    let conteoPatronesDia = {};

    let conteoPatronesGeneral = {};
...
function pegarTurnosMasivosBackend(fechaDestino, turnosCopiados) {
  try {
    ...
    if (nuevasFilas.length > 0) {
      ws.getRange(ws.getLastRow() + 1, 1, nuevasFilas.length, nuevasFilas[0].length).setValues(
        nuevasFilas
      );
    }
...
function obtenerEstadoBeneficios(fechaReferenciaStr) {
  try {
    ...
    const finQuincena = isPrimeraQuincena
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      let fechaTurno =
        rowFecha instanceof Date
          ? rowFecha
          : new Date(rowFecha.toString().trim().substring(0, 10) + "T12:00:00");
    ...
    for (let nombre in empleados) {
      ...
      if (emp.tardanzasLeves > 2)
        emp.motivos.push(`Exceso de tardanzas leves (${emp.tardanzasLeves} de 2 permitidas).`);
      if (emp.tardanzasGraves > 1)
        emp.motivos.push(`Exceso de tardanzas graves (${emp.tardanzasGraves} de 1 permitida).`);
    }
